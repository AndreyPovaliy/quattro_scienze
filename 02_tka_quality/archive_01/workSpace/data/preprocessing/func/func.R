...
# Quantity_discr("Тип труда ",dfRetro_CR_max$prof)
...
calculate_function <- function(dataframe1, list1,sample){
  ...
  for (i in 1:length(colnames(output))) {
    ...
    if(sapply(df_subset2[i], is.numeric)){
      ...
      Describe_numeric (df_subset2[,i], list1[["outputData_transl"]][i],paste("пациентов в",levels(df_subset2[,list1[["compairMain"]]])[2]," "),list1[["outputData_measure"]][i])
...
calculate_function_NoComp <- function(dataframe1, list1,sample){
