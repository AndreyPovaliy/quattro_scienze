...
# приемер:Table_numeric_Normal("возраст",df_Pillow$age, df_Pillow$group)
...
# приемер:Table_numeric_NoNormal("возраст",df_Pillow$age, df_Pillow$group)
Table_numeric_NoNormal<-function(parametr,database,dev )
{
  ...
  df2 <- subset(database, dev == levels(dev)[2])
  median2<-round(median(database, na.rm = TRUE),2)
  quan2<-summary(database, na.rm = TRUE)
...
# Quantity_discr("Девиация была ",df_Pillow$deviation)
...
Quantity_table<-function(parametr,database,vector,sep){
  ...
  Quantity_table_2(parametr,database1[[vector]],database2[[vector]])
}


#Quantity_table_2("Диагноз",dfXlsxGr1$diagnosis_primary,dfXlsxGr3$diagnosis_primary)
Quantity_table_2<-function(parametr,database1,database2){
...
# приемер:Table_numeric2("возраст",df_Pillow$age, df_Pillow$group)
...
# chapter_qual_text("Девиация",df_Pillow,"deviation","group")
...
calculate_function <- function(dataframe1, list1,sample){
  ...
  input  <- dataframe1[,c(list1[["inputData"]],list1[["compairMain"]])]
  n_Figure <- 1
  n_Table <- 1
  ...
  for (i in 1:length(colnames(input))) {
    ...
    if(sapply(input[i], is.factor)){
      chapter_qual_text(list1[["inputData_transl"]][i],input,list1[["inputData"]][i],list1[["compairMain"]])
  ...
  for (i in 1:length(colnames(input))) {
    if(sapply(input[i], is.factor)){
      ...
      try(pvalueQualitativeText(input[,i], input[,list1[["compairMain"]][1]],list1[["inputData_transl"]][i]))
    }
  }
  ...
  for (i in 1:length(colnames(input))) {
    ...
    if(sapply(input[i], is.factor)){
      ...
      string <-paste("![](img/",sample1,"/",list1[["inputData"]][i],".png)")
      ...
      nameImg <- GeomBar(input,input[,i],input[,list1[["compairMain"]][1]],list1[["inputData"]][i])
      string <-paste("img/","/",sample1,"/",list1[["inputData"]][i],".png")
      updated_string <- gsub(" ", "", string)
  ...
  subset(df_Pillow, df_Pillow[,list_Pillow[["compairMain"]][1]] == levels(df_Pillow[,list_Pillow[["compairMain"]]])[2])
  
  
  escribir(paste("\"##\" Сравнение между группами по количественным данным",sample))
  for (i in 1:length(colnames(input))) {
    df_subset1 <- subset(input, input[,list1[["compairMain"]][1]] == levels(input[,list1[["compairMain"]]])[1])
    
    if(sapply(df_subset1[i], is.numeric)){
      
      Describe_numeric (df_subset1[,i], list1[["inputData_transl"]][i],paste("пациентов в",levels(df_subset1[,list1[["compairMain"]]])[1]," "),list1[["inputData_measure"]][i])
    }
    
  }
  
  for (i in 1:length(colnames(input))) {
    df_subset2 <- subset(input, input[,list1[["compairMain"]][1]] == levels(input[,list1[["compairMain"]]])[2])
    
    if(sapply(df_subset2[i], is.numeric)){
      
      Describe_numeric (df_subset2[,i], list1[["inputData_transl"]][i],paste("пациентов в",levels(df_subset2[,list1[["compairMain"]]])[2]," "),list1[["inputData_measure"]][i])
  ...
  for (i in 1:length(colnames(input))) {
    ...
    if(sapply(input[i], is.numeric)){
      ...
      try(Table_numeric2(list1[["inputData_transl"]][i],input[,i], input[,list1[["compairMain"]][1]]))
      escribir (paste("Таблица №",as.character(n_Table),". Распределение по ",list1[["inputData_transl"]][i]," в группах X и Y.",sample))
      n_Table <- n_Table+1
    }
  ...
  for (i in 1:length(colnames(input))) {
    if(sapply(input[i], is.numeric)){
      escribir(list1[["inputData_transl"]][i])
      try(Value_numeric2(list1[["inputData_transl"]][i],input[,i], input[,list1[["compairMain"]][1]]))
    }
  }
  escribir("\n\n")
  ...
  for (i in 1:length(colnames(input))) {
    ...
    if(sapply(input[i], is.numeric)){
      ...
      escribir (updated_string)
      escribir (paste("Рисунок №",as.character(n_Figure),". Распределение по ",list1[["inputData_transl"]][i]," в группах X и Y.",sample))
      nameImg <- Violinplot(input,input[,i],input[,list1[["compairMain"]][1]],list1[["inputData"]][i],"группа")
      string <-paste("img/","/",sample1,"/",list1[["inputData"]][i],".png")
      updated_string <- gsub(" ", "", string)
      ggsave(file.path(PSDS_PATH, updated_string), 
             plot = nameImg, units = "in", width = 14, height = 8)
  ...
  escribir(paste("\n\n",toupper("результаты"),"\n\n"))
  output  <- dataframe1[,c(list1[["outputData"]],list1[["compairMain"]])]
  n_Figure <- 1
  n_Table <- 1
  escribir("\n\n")
  # описать в целом по факторам
  ...
  for (i in 1:length(colnames(output))) {
    if(sapply(output[i], is.factor)){
      chapter_qual_text(list1[["outputData_transl"]][i],output,list1[["outputData"]][i],list1[["compairMain"]])
  ...
  for (i in 1:length(colnames(output))) {
    if(sapply(output[i], is.factor)){
      ...
      try(pvalueQualitativeText(output[,i], output[,list1[["compairMain"]][1]],list1[["outputData_transl"]][i]))
    }
  }
  ...
  for (i in 1:length(colnames(output))) {
    ...
    if(sapply(output[i], is.factor)){
      sample1 <- gsub(" ", "_", sample)
      string <-paste("![](img/",sample1,"/",list1[["outputData"]][i],".png)")
      updated_string <- gsub(" ", "", string)
      escribir (updated_string)
      escribir (paste("Рисунок №",as.character(n_Figure),". Распределение по ",list1[["outputData_transl"]][i]," в группах X и Y.",sample))
      nameImg <- GeomBar(output,output[,i],output[,list1[["compairMain"]][1]],list1[["outputData"]][i])
      string <-paste("img/","/",sample1,"/",list1[["outputData"]][i],".png")
      updated_string <- gsub(" ", "", string)
      ggsave(file.path(PSDS_PATH, updated_string), 
             plot = nameImg, units = "in", width = 14, height = 8)
  ...
  escribir(paste("\"##\" Сравнение между группами по количественным данным",sample))
  for (i in 1:length(colnames(output))) {
    df_subset1 <- subset(output, output[,list1[["compairMain"]][1]] == levels(output[,list1[["compairMain"]]])[1])
    
    if(sapply(df_subset1[i], is.numeric)){
      
      Describe_numeric (df_subset1[,i], list1[["outputData_transl"]][i],paste("пациентов в",levels(df_subset1[,list1[["compairMain"]]])[1]," "),list1[["outputData_measure"]][i])
    }
    
  }
  escribir("\n\n")
  for (i in 1:length(colnames(output))) {
    df_subset2 <- subset(output, output[,list1[["compairMain"]][1]] == levels(df_subset2[,list1[["compairMain"]]])[2])
    
    if(sapply(df_subset2[i], is.numeric)){
      
      Describe_numeric (df_subset2[,i], list1[["outputData_transl"]][i],paste("пациентов в",levels(df_subset2[,list1[["compairMain"]]])[1]," "),list1[["outputData_measure"]][i])
  ...
  for (i in 1:length(colnames(output))) {
    ...
    if(sapply(output[i], is.numeric)){
      ...
      try(Table_numeric2(list1[["outputData_transl"]][i],output[,i], output[,list1[["compairMain"]][1]]))
      escribir (paste("Таблица №",as.character(n_Table),". Распределение по ",list1[["outputData_transl"]][i]," в группах X и Y.",sample))
      n_Table <- n_Table+1
    }
  ...
  for (i in 1:length(colnames(output))) {
    if(sapply(output[i], is.numeric)){
      escribir(list1[["outputData_transl"]][i])
      try(Value_numeric2(list1[["outputData_transl"]][i],output[,i], output[,list1[["compairMain"]][1]]))
    }
  }
  escribir("\n\n")
  # визуализация группы по номерам
  ...
  for (i in 1:length(colnames(output))) {
    ...
    if(sapply(output[i], is.numeric)){
      ...
      escribir (updated_string)
      escribir (paste("Рисунок №",as.character(n_Figure),". Распределение по ",list1[["outputData_transl"]][i]," в группах X и Y.",sample))
      nameImg <- Violinplot(output,output[,i],output[,list1[["compairMain"]][1]],list1[["outputData"]][i],"группа")
      string <-paste("img/","/",sample1,"/",list1[["outputData"]][i],".png")
      updated_string <- gsub(" ", "", string)
      ggsave(file.path(PSDS_PATH, updated_string), 
             plot = nameImg, units = "in", width = 14, height = 8)
